...
class GreedyBFS(private val array: GameArray, val objects: LinkedList<GameObject> = LinkedList()) : Search {
    ...
    override fun findAPathToPacMan(fromPair: Pair, toPair: Pair, dir: DIRECTION): LinkedList<Node> {
        ...
        while (!goalFound) {
            currentNode = getLowestFromOpenList(toPair)
    ...
    private fun getLowestFromOpenList(target: Pair): Node {
        return open.minBy { abs(it.x - target.first) + abs(it.y - target.second) }
    }
